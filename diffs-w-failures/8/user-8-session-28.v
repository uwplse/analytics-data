...
Notation "'let_' p \226\134\144 c1 ; c2" := (comp p c1 c2)
...
Notation "'let_' ( ( p1 , p2 ) , p3 ) \226\134\144 c1 ; c2" :=
  (compose c1
     (fun x => let (y, p3) := wproj x in let (p1, p2) := wproj y in c2))
  (at level 14, right associativity) : circ_scope.
Notation "'let_' ( p1 , p2 , p3 ) \226\134\144 c1 ; c2" :=
  (compose c1
     (fun x => let (y, p3) := wproj x in let (p1, p2) := wproj y in c2))
...
Notation "'let_' ( p1 , ( p2 , p3 ) ) \226\134\144 c1 ; c2" :=
  (compose c1
     (fun x => let (p1, y) := wproj x in let (p2, p3) := wproj y in c2))
  (at level 14, right associativity) : circ_scope.
Notation "'let_' ( p1 , ( p2 , ( p3 , p4 ) ) ) \226\134\144 c1 ; c2" :=
  (compose c1
     (fun x =>
...
Notation "'let_' ( ( p1 , p2 ) , ( p3 , p4 ) ) \226\134\144 c1 ; c2" :=
  ...
  (at level 14, right associativity) : circ_scope.
Notation
  "'let_' ( p1 , ( p2 , ( p3 , ( p4 , ( p5 , p6 ) ) ) ) ) \226\134\144 c1 ; c2" :=
  (compose c1
     (fun x =>
      let (p1, y) := wproj x in
      let (p2, z) := wproj y in
...
Notation "'gate_' ( ( p1 , p2 ) , ( p3 , p4 ) ) \226\134\144 g @ p ; c2" :=
  ...
  (at level 14, right associativity) : circ_scope.
Notation
  "'gate_' ( p1 , ( p2 , ( p3 , ( p4 , ( p5 , p6 ) ) ) ) ) \226\134\144 g @ p ; c2" :=
  (gate g p
     (fun x =>
      let (p1, y) := wproj x in
      let (p2, z) := wproj y in
      ...
      gate discard p2
        (fun _ =>
         gate discard p3
           (fun _ =>
            gate discard p4
              (fun _ => gate discard p5 (fun _ => gate discard p6))))))
  (at level 14, right associativity) : circ_scope.
...
Ltac
 destruct_merges := repeat match goal with
                           | H:_ \226\169\181 _ \226\136\153 _ |- _ => destruct H
                           end.
Definition cnot12 : Square_Box (Qubit \226\138\151 Qubit \226\138\151 Qubit) :=
  box_ (p0, p1, p2)
  \226\135\146 (gate_ (p3, p4)\226\134\144 CNOT @ (p1,, p2); output (p0,, p3,, p4)).
