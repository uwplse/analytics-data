...
Notation "\194\172 b" := (negb b) (at level 10).
Infix "\226\138\149" := xorb (at level 20).
Search -xorb -andb.
Lemma xorb_nb_b : forall b, \194\172 b \226\138\149 b = true.
Proof.
(destruct b; easy).
Qed.
Lemma xorb_b_nb : forall b, b \226\138\149 \194\172 b = true.
Proof.
(destruct b; easy).
Qed.
Lemma xorb_involutive_l : forall b b', b \226\138\149 (b \226\138\149 b') = b'.
Proof.
(destruct b, b'; easy).
Qed.
Lemma xorb_involutive_r : forall b b', (b \226\138\149 b') \226\138\149 b' = b.
Proof.
(destruct b, b'; easy).
Qed.
Lemma andb_xorb_dist : forall b b1 b2, b && b1 \226\138\149 b2 = (b && b1) \226\138\149 (b && b2).
Proof.
(destruct b, b1, b2; easy).
Qed.
...
Ltac bdestruct\206\169 X := bdestruct X; simpl; try lia.
Lemma if_dist :
  forall (A B : Type) (b : bool) (f : A -> B) (x y : A),
  f (if b then x else y) = (if b then f x else f y).
Proof.
(destruct b; reflexivity).
Qed.
Lemma if_dist2 :
  forall (A B C : Type) (b : bool) (f : A -> B -> C) (x y : A) (z : B),
  f (if b then x else y) z = (if b then f x z else f y z).
Proof.
(destruct b; reflexivity).
Qed.
Lemma f_equal_gen :
  forall {A} {B} (f g : A -> B) a b, f = g -> a = b -> f a = g b.
Proof.
(intros).
subst.
reflexivity.
...
Qed.
Lemma f_equal2_inv :
  forall {A} {B} {C} x y (f g : A -> B -> C), f = g -> f x y = g x y.
Proof.
(intros).
(rewrite H).
easy.
...
Fixpoint update_at {A} (ls : list A) (i : nat) (a : A) : 
list A :=
  match ls, i with
  | [], _ => []
  | _ :: ls', 0 => a :: ls'
...
Lemma firstn_repeat_le :
  forall A (a : A) m n, (m <= n)%nat -> firstn m (repeat a n) = repeat a m.
Proof.
(induction m; trivial).
(intros n L).
...
reflexivity.
Qed.
Lemma firstn_repeat_ge :
  forall A (a : A) m n, (m >= n)%nat -> firstn m (repeat a n) = repeat a n.
Proof.
(intros A a m n H).
...
Qed.
Lemma firstn_repeat :
  forall A (a : A) m n, firstn m (repeat a n) = repeat a (min m n).
Proof.
(intros).
(bdestruct (m <=? n)).
-
...
Qed.
Lemma skipn_repeat :
  forall A (a : A) m n, skipn m (repeat a n) = repeat a (n - m).
Proof.
...
Notation "ls1 \226\138\134 ls2" := (subset ls1 ls2 = true) (at level 30).
Fixpoint disjoint (ls1 ls2 : list nat) : bool :=
  match ls1 with
  | [] => true
  | a :: ls1' => negb (inb a ls2) && disjoint ls1' ls2
  end.
Notation "ls1 \226\138\165 ls2" := (disjoint ls1 ls2 = true) (at level 30).
Lemma disjoint_nil_l : forall ls, nil \226\138\165 ls.
Proof.
reflexivity.
Qed.
Lemma disjoint_nil_r : forall ls, ls \226\138\165 nil.
Proof.
...
Lemma lookup_app :
  forall x ls1 ls2,
  lookup x (ls1 ++ ls2) =
  (if inb x ls1 then lookup x ls1 else (lookup x ls2 + length ls1)%nat).
Proof.
...
Qed.
Lemma subset_app :
  forall ls1 ls2 ls, (ls1 ++ ls2) \226\138\134 ls -> ls1 \226\138\134 ls /\ ls2 \226\138\134 ls.
Proof.
...
Ltac
 simpl_rewrite' lem :=
  ...
  specialize lem as H; simpl in H; rewrite <- H; clear H.
Ltac
 simpl_rewrite_h lem hyp :=
  let H := fresh "H" in
  specialize lem as H; simpl in H; rewrite <- H in hyp; clear H.
Ltac
 apply_with_obligations H :=
  match goal with
  | |- ?P ?a =>
        match type of H with
        | ?P ?a' => replace a with a'; [ apply H |  ]; trivial
        end
  | |- ?P ?a ?b =>
        match type of H with
        | ?P ?a' ?b' =>
            replace a with a'; [ replace b with b'; [ apply H |  ] |  ];
             trivial
        end
  | |- ?P ?a ?b ?c =>
        match type of H with
        | ?P ?a' ?b' ?c' =>
            replace a with a';
             [ replace b with b'; [ replace c with c'; [ apply H |  ] |  ]
             |  ]; trivial
        end
  | |- ?P ?a ?b ?c ?d =>
        match type of H with
        | ?P ?a' ?b' ?c' ?d' =>
            replace a with a';
             [ replace b with b';
                [ replace c with c'; [ replace d with d'; [ apply H |  ] |  ]
                |  ]
             |  ]; trivial
        end
  | |- ?P ?a ?b ?c ?d ?e =>
...
Tactic Notation "gen" ident(X1) ident(X2) := (gen X2; gen X1).
