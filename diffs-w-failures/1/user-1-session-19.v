...
Theorem test_assoc p1 p2 p3 : p1 * p2 * p3 ===> p1 * (p2 * p3).
Proof.
cancel.
Qed.
Theorem test_reorder p1 p2 p3 : p1 * p2 * p3 ===> p1 * (p3 * p2).
Proof.
cancel.
Qed.
Theorem test_collapse_emp p1 : emp * p1 ===> p1 * emp * emp.
Proof.
cancel.
Qed.
Theorem test_lift_prop p1 p2 p3 Q : p1 * p2 * p3 * [Q] ===> p1 * (p3 * p2).
Proof.
norm.
(match goal with
 | H:Q |- _ => reflexivity
...
Qed.
Theorem test_norm_repeat p1 p2 : p1 * p2 ===> p2 * p1.
Proof.
norm.
...
auto.
Qed.
Theorem test_cancel_exact_prop p1 (P : Prop) : p1 * [P] ===> p1 * [P].
Proof.
cancel.
Qed.
Theorem test_cancel_hyp p1 p2 p3 p4 :
  p4 * p1 ===> p3 -> p1 * p2 * p4 ===> p2 * p3.
Proof.
(intros).
(norm_with H).
(rewrite H).
...
reflexivity.
Qed.
Theorem test_lift_hyp1 (P : Prop) p m : predApply ([P] * p) m -> P.
Proof.
intro H.
(norm_hyp H).
assumption.
...
Theorem test_demo A V (p1 p2 p3 : pred A V) (P Q : Prop) :
  P -> p1 * (p2 * p3 * emp) * [Q] ===> [P] * p1 * emp * (p3 * p2).
Proof.
Demo.norm.
Demo.simpl_flatten.
Demo.simpl_sorting.
