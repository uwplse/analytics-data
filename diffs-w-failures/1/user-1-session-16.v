...
Lemma array_read l : mem_read (array l) = index l.
Proof.
reflexivity.
Qed.
Hint Rewrite array_read : array.
Lemma array_inj l1 l2 : array l1 = array l2 -> l1 = l2.
Proof.
...
Definition ptsto_array (n : nat) (x : list) : pred :=
  mkPred
    (fun m =>
     forall i, (i < n -> m i = None) /\ (i >= n -> m i = index x (i - n))).
Theorem ptsto_array_subslice n x :
  forall l, ptsto_array n x (array l) -> subslice l n (length x) = x.
Proof.
(unfold ptsto_array; propositional).
(apply index_ext_eq; intros i).
(simpl in *).
(destruct (lt_dec i (length x)); autorewrite with array in *).
...
Theorem ptsto_array_oob_small n x m :
  ptsto_array n x m -> forall i, i < n -> m i = None.
Proof.
(unfold ptsto_array; propositional).
(specialize (H i); propositional).
Qed.
Theorem ptsto_array_oob_large n x m :
  ptsto_array n x m -> forall i, i >= n + length x -> m i = None.
Proof.
(unfold ptsto_array; propositional).
(specialize (H i); propositional).
specialize (H1 _).
...
Definition array_at_mem n x : mem :=
  mkMem (fun i => if le_dec n i then index x (i - n) else None).
Theorem ptsto_array_mem n x m : ptsto_array n x m <-> m = array_at_mem n x.
Proof.
...
Lemma ptsto_array_nil n : ptsto_array n [] === emp.
Proof.
...
(apply mem_ext_eq; intros i; simpl).
(subst; simpl).
(destruct (le_dec n i), (Instances.equal n i), (le_dec (S n) i); try omega;
  auto).
...
-
(replace (n1 + 0) with n1 by omega; cancel).
-
(rewrite <- IHx1).
