...
Fixpoint insert (x : A) (xs : list A) : list A :=
  match xs with
  | nil => [x]
  | x' :: xs' => if x <? x' then x :: x' :: xs' else x' :: insert x xs'
  end.
Hint Constructors Permutation: core.
Theorem inserted_permutation :
  forall x xs, Permutation (x :: xs) (insert x xs).
Proof.
(induction xs; simpl; eauto).
(destruct (x <? a); eauto).
Qed.
Fixpoint insert_sort (xs : list A) (acc : list A) : 
list A :=
  match xs with
  | nil => acc
...
Theorem insert_sort_permutation :
  forall xs acc, Permutation (xs ++ acc) (insert_sort xs acc).
Proof.
...
Definition sortBy {A} (key : A -> option nat) (xs : list A) : 
  list A :=
  sort
    (fun x y =>
     Nat.leb match key x with
             | Some n => S n
             | None => 0
             end match key y with
                 | Some n => S n
                 | None => 0
                 end) xs.
Theorem sortBy_permutation :
  forall A key (xs : list A), Permutation xs (sortBy key xs).
Proof.
