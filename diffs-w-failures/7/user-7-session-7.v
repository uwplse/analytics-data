...
(destruct t'; try (solve [ simpl; reflexivity ])).
Qed.
Lemma unite_pairs_atom_union : forall t t1 t2 : ty, atom_type t -> unite_pairs t (TUnion t1 t2) = TUnion (unite_pairs t t1) (unite_pairs t t2).
Proof.
(intros t t1 t2 Hat).
(induction Hat; reflexivity).
Qed.
Lemma unite_pairs_union_atom : forall t1 t2 t : ty, atom_type t -> unite_pairs (TUnion t1 t2) t = TUnion (unite_pairs t1 t) (unite_pairs t2 t).
Proof.
(intros t1 t2 t Hat).
(induction Hat; reflexivity).
Qed.
...
-
(rewrite (mk_nf_nf__equal _ Hnf) in IHsub_r).
