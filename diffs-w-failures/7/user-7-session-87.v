...
Close Scope btjmi.
Set Silent.
Theorem sem_sub_i__sem_sub : forall t1 t2 : ty, (||- [t1]<= [t2])%btjmi -> (||- [t1]<= [t2])%btjm.
Proof.
Unset Silent.
(intros t1 t2 Hsemi).
(apply sub_d__semantic_sound).
(apply sub_d__semantic_i_complete).
Set Silent.
assumption.
Unset Silent.
Qed.
Theorem sem_sub__sem_sub_i : forall t1 t2 : ty, (||- [t1]<= [t2])%btjm -> (||- [t1]<= [t2])%btjmi.
Set Silent.
Proof.
(intros t1 t2 Hsemdec).
Unset Silent.
(apply sub_d__semantic_i_sound).
Set Silent.
