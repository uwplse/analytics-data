...
Time Qed.
Time Canonical Structure authC := OfeT (auth A) auth_ofe_mixin.
Time #[global]
...
Time Qed.
Time #[global]
Instance auth_ofe_discrete : (OfeDiscrete A \226\134\146 OfeDiscrete authC).
Time Proof.
Time (intros ? [? ?]; apply _).
Time Qed.
Time End ofe.
Time Arguments authC : clear implicits.
Time Section cmra.
...
Time
(intros [[[? ?]| ] ?] [[[? ?]| ] ?];
  try <ssreflect_plugin::ssrtclintros@0> apply Auth_proper =>//=;
  try by rewrite cmra_morphism_op).
Time by rewrite -Some_op pair_op cmra_morphism_op.
Time Qed.
Time
Definition authC_map {A} {B} (f : A -n> B) : authC A -n> authC B :=
  CofeMor (auth_map f).
Time Lemma authC_map_ne A B : NonExpansive (@authC_map A B).
Time Proof.
Time
(intros n f f' Hf [[[]| ]]; repeat constructor; try naive_solver;
  apply agreeC_map_ne; auto).
Time Qed.
Time #[program]
...
by
 intros ? A1 ? A2 ? B1 ? B2 ? n f g Hfg;
  apply authC_map_ne, urFunctor_contractive.
Time Qed.
Time #[program]
