...
Definition any {A} {B} {T} : relation A B T := fun x y => True.
Definition none {A} {B} {T} : relation A B T := fun x y => False.
Definition reads {A} {T} (f : A -> T) : relation A A T :=
  fun x y => Val x (f x) = y.
Definition puts {A} (f : A -> A) : relation A A unit :=
  fun x y => y = Val (f x) tt.
Definition error {A} {B} {T} : relation A B T := fun _ r => r = Err.
...
Proof.
(intros).
(split; apply and_then_monotonic; eauto).
Qed.
...
Proof.
(intros).
(split; apply and_then_monotonic; eauto; intros; reflexivity).
Qed.
...
Proof.
t.
(destruct_return; t; intuition congruence).
Qed.
...
Proof.
t.
(induction H0; intuition eauto; t).
Qed.
#[global]
...
t.
Qed.
Theorem from_none A B T (r : relation A B T) : none ---> r.
Proof.
t.
Qed.
...
(destruct_return; t).
Qed.
Theorem none_plus_r A B T1 (p : relation A B T1) : p + none <---> p.
Proof.
t.
Qed.
Theorem none_plus_l A B T1 (p : relation A B T1) : none + p <---> p.
Proof.
t.
Qed.
...
t.
-
(induction H0; eauto; edestruct (H o1); intuition; t).
-
(induction H0; eauto; edestruct (H o1); intuition; t).
Qed.
...
Proof.
(t; destruct_return; t; induction H; eauto; t; destruct_return; t).
Qed.
#[global]
...
Proof.
t.
(destruct_return; t; specialize (H o1); t).
Qed.
...
(destruct_return; t).
*
(remember (Val y o1) as z eqn:Heq ).
revert y o1 H0 Heq.
revert H.
(induction 1; intros).
**
...
(remember (Val y tt) as ret eqn:Heq ).
gen y Heq.
(induction Hstar; subst; t).
**
(specialize (IHHstar _ _ _); t).
left.
right.
(do 2 eexists; split; eauto).
(eapply bind_star_one_more_err).
eauto.
*
t.
**
(set (ret := @Err A unit)).
(replace Err with ret in H at 1; eauto).
(remember ret as ret' eqn:Heq ).
(unfold ret in Heq).
(induction H).
***
congruence.
...
gen x.
(unshelve (induction H0; t); try exact tt).
(unshelve (specialize (IHbind_star _ _); t); try exact tt).
Qed.
...
Proof.
(intros Hr Hseq).
(remember (Val z o2) as ret eqn:Heq ).
revert x z o1 o2 Hr Heq.
...
Proof.
(intros Hstar).
(remember (Val y o) as ret eqn:Heq ).
revert y o Heq.
(induction Hstar; t).
Qed.
...
gen Heq.
(unshelve (induction H1; intros; t); eauto).
left.
left.
(edestruct (@seq_star_r_one_more_valid_left A _ r); eauto; t).
*
...
revert y b o1 Heq H1.
(induction H0; intros; t).
