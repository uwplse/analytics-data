...
(intros n P Q HPQ; <ssreflect_plugin::ssrtclintros@0> split =>i x ? ?;
  apply HPQ; auto).
Time Qed.
Time Canonical Structure uPredC : ofeT := OfeT (uPred M) uPred_ofe_mixin.
Time #[program]
Definition uPred_compl : Compl uPredC :=
...
Time Qed.
Time #[global, program]
Instance uPred_cofe : (Cofe uPredC) := {| compl := uPred_compl |}.
Time Next Obligation.
...
Time Qed.
Time End cofe.
Time Arguments uPredC : clear implicits.
...
Time Qed.
Time
Definition uPredC_map {M1 M2 : ucmraT} (f : M2 -n> M1) 
  `{!CmraMorphism f} : uPredC M1 -n> uPredC M2 :=
  CofeMor (uPred_map f : uPredC M1 \226\134\146 uPredC M2).
Time
Lemma uPredC_map_ne {M1 M2 : ucmraT} (f g : M2 -n> M1) 
  `{!CmraMorphism f} `{!CmraMorphism g} n :
  f \226\137\161{n}\226\137\161 g \226\134\146 uPredC_map f \226\137\161{n}\226\137\161 uPredC_map g.
Time Proof.
Time
...
Time Qed.
Time #[program]
Definition uPredCF (F : urFunctor) : cFunctor :=
  {|
  cFunctor_car := fun A _ B _ => uPredC (urFunctor_car F B A);
  cFunctor_map := fun A1 _ A2 _ B1 _ B2 _ fg =>
                  uPredC_map (urFunctor_map F (fg.2, fg.1)) |}.
Time Next Obligation.
Time (intros F A1 ? A2 ? B1 ? B2 ? n P Q HPQ).
Time (apply uPredC_map_ne, urFunctor_ne; split; by apply HPQ).
Time Qed.
Time Next Obligation.
...
Instance uPredCF_contractive  F:
 (urFunctorContractive F \226\134\146 cFunctorContractive (uPredCF F)).
Time Proof.
Time (intros ? A1 ? A2 ? B1 ? B2 ? n P Q HPQ).
Time (apply uPredC_map_ne, urFunctor_contractive).
...
Time (eapply uPred_holds_ne, Hlim, HP; eauto using conv_compl).
Time Qed.
...
Time by unseal.
Time Qed.
Time Lemma prop_ext P Q : \226\150\160 ((P -\226\136\151 Q) \226\136\167 (Q -\226\136\151 P)) \226\138\162 P \226\137\161 Q.
Time Proof.
...
Time Qed.
Time
Lemma fun_ext {A} {B : A \226\134\146 ofeT} (g1 g2 : ofe_fun B) :
  (\226\136\128 i, g1 i \226\137\161 g2 i) \226\138\162 g1 \226\137\161 g2.
Time Proof.
Time by unseal.
Time Qed.
Time
Lemma sig_eq {A : ofeT} (P : A \226\134\146 Prop) (x y : sigC P) :
