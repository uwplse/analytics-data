...
Time Arguments agreeC : clear implicits.
...
Definition agreeC_map {A} {B} (f : A -n> B) : agreeC A -n> agreeC B :=
  CofeMor (agree_map f : agreeC A \226\134\146 agreeC B).
Time Instance agreeC_map_ne  A B: (NonExpansive (@agreeC_map A B)).
Time Proof.
Time
...
Time Qed.
Time #[program]
Definition agreeRF (F : cFunctor) : rFunctor :=
  {|
  rFunctor_car := fun A _ B _ => agreeR (cFunctor_car F A B);
  rFunctor_map := fun A1 _ A2 _ B1 _ B2 _ fg =>
                  agreeC_map (cFunctor_map F fg) |}.
Time Next Obligation.
Time (intros ? A1 ? A2 ? B1 ? B2 ? n ? ? ?; simpl).
Time by apply agreeC_map_ne, cFunctor_ne.
Time Qed.
Time Next Obligation.
...
Instance agreeRF_contractive  F:
 (cFunctorContractive F \226\134\146 rFunctorContractive (agreeRF F)).
Time Proof.
Time (intros ? A1 ? A2 ? B1 ? B2 ? n ? ? ?; simpl).
Time by apply agreeC_map_ne, cFunctor_contractive.
Time Qed.
