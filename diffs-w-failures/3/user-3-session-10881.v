...
Lemma read_op_ok :
  forall i a,
  proc_hspec TDBaseDynamics (td.read i a)
    (op_spec TDBaseDynamics (op_read i a)).
Proof.
(intros).
...
Lemma write_op_ok :
  forall i a b,
  proc_hspec TDBaseDynamics (td.write i a b)
    (op_spec TDBaseDynamics (op_write i a b)).
Proof.
(intros).
(eapply op_spec_sound).
Qed.
...
Definition other (i : diskId) := match i with
                                 | d0 => d1
                                 | d1 => d0
                                 end.
Definition read_spec (i : diskId) (a : addr) :
  _ -> Specification _ unit State :=
  fun '(d, F) state =>
  {|
  pre := get_disk i state ?|= eq d /\ get_disk (other i) state ?|= F;
  post := fun state' r =>
          match r with
          | Working v =>
              get_disk i state' ?|= eq d /\
              get_disk (other i) state' ?|= F /\ index d a ?|= eq v
          | Failed =>
              get_disk i state' ?|= missing /\
              get_disk (other i) state' ?|= F
          end;
  alternate := fun state' r =>
               get_disk i state' ?|= eq d /\ get_disk (other i) state' ?|= F |}.
...
Definition size_spec (i : diskId) : _ -> Specification _ unit _ :=
  ...
  post := fun state' r =>
          ...
          | Working n =>
              get_disk i state' ?|= eq d /\
              get_disk (other i) state' ?|= F /\ n = length d
          | Failed =>
              get_disk i state' ?|= missing /\
              get_disk (other i) state' ?|= F
          end;
...
Ltac
 inv_bg :=
  match goal with
  | H:bg_failure _ _ _ |- _ => inversion H; subst; clear H
  end.
...
Qed.
Lemma identity_unfold S (s s' : S) T (v : T) : identity s s' v -> s' = s.
Proof.
...
Ltac
 prim :=
  intros; eapply proc_hspec_impl; [ unfold spec_impl | eauto ]; eexists;
...
Theorem read_ok :
  forall i a dF, proc_hspec TDBaseDynamics (td.read i a) (read_spec i a dF).
Proof.
(unshelve prim; eauto).
Qed.
Ltac
...
Theorem write_ok :
  forall i a v dF,
  proc_hspec TDBaseDynamics (td.write i a v) (write_spec i a v dF).
Proof.
...
Qed.
Theorem size_ok :
  forall i dF, proc_hspec TDBaseDynamics (td.size i) (size_spec i dF).
Proof.
unshelve prim.
Qed.
End specs.
