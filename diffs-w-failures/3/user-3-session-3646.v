...
Definition refine_spec T R (spec : Specification T R AState) :
  ...
  fun s cs =>
  {|
  pre := absr s cs tt /\ (spec s).(pre);
  post := fun cs' r => exists s', absr s' cs' tt /\ (spec s).(post) s' r;
  alternate := fun cs' r =>
               exists s', absr s' cs' tt /\ (spec s).(alternate) s' r |}.
...
(simpl in Hexec).
(edestruct Hexec as (sA', (?, ?)); eauto).
{
(unfold refine_spec; simpl).
(split; auto).
(eapply Hpre; eauto).
}
...
Lemma proc_rspec_recovery_refines_crash_step (rec : proc C_Op unit) 
  ...
  (forall sA sC sC',
   absr sA sC tt -> crash_step c_sem sC sC' tt -> (spec sA sC').(pre)) ->
...
(destruct Hstep
  as ([], (sCmid, (Hcrash, ([], (sCmid', (Hcrash_star, Hrec))))))).
...
-
left.
(edestruct Hinit_post as (sA', ?)).
{
(eapply Hproc; eauto).
}
(do 2 (exists tt,sA'; split; intuition)).
firstorder.
-
right.
(exists tt,sC'; do 2 eexists; intuition).
Qed.
End Abstraction.
