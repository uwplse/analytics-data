...
Proof.
revert o a1 a2.
(induction n; intros o a1 a2).
-
(simpl).
(inversion 1; subst).
(apply bind_star_pure).
-
(simpl).
destruct_return.
*
...
Proof.
revert o a a' o'.
(induction n; intros o a a' o').
{
reflexivity.
}
split.
-
(intros (omid, (amid, (Hr, Hn)))).
(eapply IHn in Hn).
clear - Hr Hn.
revert o a amid omid a' o' Hr Hn.
(induction n; intros).
*
(inversion Hn; subst).
...
(assert (Himpl' : and_then x p ---> x)).
{
setoid_rewrite  <- Himpl at 2.
Right.
}
(eapply bind_star_rep_n_ind).
(intros n).
(induction n).
-
(simpl).
