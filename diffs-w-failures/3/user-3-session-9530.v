...
Definition read_spec (i : diskId) (a : addr) : Specification _ _ unit _ :=
  ...
  post := fun r state' =>
          ...
          | Working v =>
              ...
              get_disk (other i) state' ?|= F /\ diskGet d a ?|= eq v
          | Failed =>
...
Definition write_spec (i : diskId) (a : addr) (b : block) :
  Specification _ (DiskResult unit) unit _ :=
  fun '(d, F) state =>
  {|
  pre := get_disk i state ?|= eq d /\ get_disk (other i) state ?|= F;
  post := fun r state' =>
...
Definition size_spec (i : diskId) : Specification _ _ unit _ :=
  ...
  post := fun r state' =>
          ...
          | Working n =>
              get_disk i state' ?|= eq d /\
              get_disk (other i) state' ?|= F /\ n = diskSize d
          | Failed =>
              get_disk i state' ?|= missing /\
              get_disk (other i) state' ?|= F
          end;
...
Axiom (init_ok : init_abstraction init recover abstr inited_any).
Axiom
  (read_ok : forall i a, proc_spec (read_spec i a) (read i a) recover abstr).
Axiom
  (write_ok :
