...
Theorem impl_spec_exec `(spec : Specification T R State) 
  (r : relation _ _ _) :
  (forall s,
   (spec s).(pre) -> forall ret, r s ret -> ret_matches (spec s).(post) ret) <->
  r ---> spec_exec spec.
Proof.
firstorder.
Qed.
Theorem impl_spec_aexec `(spec : Specification T R State)
  (r : relation _ _ _) :
  (forall s,
   (spec s).(pre) ->
   forall ret, r s ret -> ret_matches (spec s).(alternate) ret) <->
  r ---> spec_aexec spec.
Proof.
firstorder.
Qed.
...
Theorem spec_impl_rel `(spec1 : Specification T R State) 
  spec2 :
...
rew Hp_ok.
(change_no_check
   (let! v <- spec_exec spec; let! _ <- exec_crash (rx v);
    exec_recover sem rec) with (let! v <- spec_exec spec; rexec (rx v) rec)).
...
Theorem ret_hspec T `(spec : Specification T unit State) 
  ...
  (forall state,
   ...
   post (spec state) state v /\
   (forall state',
    crash_step state (Val state' tt) -> alternate (spec state) state' tt)) ->
