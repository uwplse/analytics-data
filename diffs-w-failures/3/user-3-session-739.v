...
Time Qed.
Time Canonical Structure exclC : ofeT := OfeT (excl A) excl_ofe_mixin.
Time #[global]Instance excl_cofe  `{!Cofe A}: (Cofe exclC).
Time Proof.
...
Time Qed.
Time #[global]
Instance excl_ofe_discrete : (OfeDiscrete A \226\134\146 OfeDiscrete exclC).
Time Proof.
...
Time Qed.
Time End excl.
Time Arguments exclC : clear implicits.
...
Definition exclC_map {A} {B} (f : A -n> B) : exclC A -n> exclC B :=
  CofeMor (excl_map f).
Time Instance exclC_map_ne  A B: (NonExpansive (@exclC_map A B)).
Time Proof.
Time by intros n f f' Hf []; constructor; apply Hf.
Time Qed.
Time #[program]
Definition exclRF (F : cFunctor) : rFunctor :=
  {|
  rFunctor_car := fun A _ B _ => exclR (cFunctor_car F A B);
  rFunctor_map := fun A1 _ A2 _ B1 _ B2 _ fg => exclC_map (cFunctor_map F fg) |}.
Time Next Obligation.
Time (intros F A1 ? A2 ? B1 ? B2 ? n x1 x2 ? ?).
Time by apply exclC_map_ne, cFunctor_ne.
Time Qed.
Time Next Obligation.
...
Instance exclRF_contractive  F:
 (cFunctorContractive F \226\134\146 rFunctorContractive (exclRF F)).
Time Proof.
Time (intros A1 ? A2 ? B1 ? B2 ? n x1 x2 ? ?).
Time by apply exclC_map_ne, cFunctor_contractive.
Time Qed.
