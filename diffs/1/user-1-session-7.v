...
From Tactical Require Import Propositional SimplMatch.
Require Import SepLogic.Instances.
Set Implicit Arguments.
Set Warnings "-undeclared-scope".
Section Memory.
Context (A V : Type).
Record mem := mkMem {mem_read :> A -> option V}.
Implicit Types (a : A) (v : V) (m : mem).
Definition empty : mem := mkMem (fun _ => None).
...
congruence.
Qed.
Theorem disjoint_sym m1 m2 : m1 # m2 <-> m2 # m1.
Proof.
firstorder.
Qed.
Theorem disjoint_sym1 m1 m2 : m1 # m2 -> m2 # m1.
Proof.
firstorder.
Qed.
...
Theorem mem_ext_eq m1 m2 : (forall a, m1 a = m2 a) -> m1 = m2.
Proof.
(intros).
(destruct m1, m2).
f_equal.
...
Ltac
 t :=
  autounfold with mem; simpl;
   repeat
    match goal with
    | m:mem |- _ => destruct m as m
...
t.
Qed.
Theorem upd_ne m a v a' : a <> a' -> upd m a v a' = m a'.
Proof.
t.
Qed.
Theorem upd_upd m a v v' : upd (upd m a v) a v' = upd m a v'.
Proof.
t.
Qed.
Theorem upd_upd_ne m a v a' v' :
  a <> a' -> upd (upd m a v) a' v' = upd (upd m a' v') a v.
Proof.
t.
Qed.
Theorem empty_disjoint1 m : m # empty.
Proof.
t.
Qed.
Theorem empty_disjoint2 m : empty # m.
Proof.
t.
Qed.
Theorem empty_union1 m : m + empty = m.
Proof.
t.
Qed.
Theorem empty_union2 m : empty + m = m.
Proof.
t.
Qed.
Theorem disjoint_union_comm m1 m2 : m1 # m2 -> m1 + m2 = m2 + m1.
Proof.
t.
Qed.
Theorem union_disjoint1 m m1 m2 : m # m1 + m2 -> m # m1.
Proof.
t.
specialize (H _ _ _).
(replace (m1 x) in *; t).
Qed.
Theorem union_disjoint2 m m1 m2 : m # m1 + m2 -> m # m2.
Proof.
t.
specialize (H _ _ _).
(destruct_with_eqn m1 x; t).
Qed.
Theorem union_disjoint_elim m m1 m2 : m # m1 + m2 -> m # m1 /\ m # m2.
Proof.
(split; eauto using union_disjoint1, union_disjoint2).
Qed.
Theorem union_disjoint_intro m m1 m2 : m # m1 -> m # m2 -> m # m1 + m2.
Proof.
t.
(destruct_with_eqn m1 x; t).
Qed.
Theorem union_assoc m1 m2 m3 : m1 + m2 + m3 = m1 + (m2 + m3).
Proof.
t.
Qed.
...
Theorem disjoint_different_singleton m a v v' :
  m # singleton a v -> m # singleton a v'.
Proof.
t.
(destruct matches in *;
  repeat
...
Lemma disjoint_from_singleton a v m :
  m a = None -> disjoint (singleton a v) m.
Proof.
t.
(destruct (a == x); try congruence).
Qed.
Lemma disjoint_singleton_oob a v m : disjoint (singleton a v) m -> m a = None.
Proof.
t.
...
(exfalso; eauto).
Qed.
Theorem singleton_eq a v : singleton a v a = Some v.
Proof.
t.
Qed.
Theorem singleton_ne a v a' : a <> a' -> singleton a v a' = None.
Proof.
t.
Qed.
End Memory.
...
Notation "m [ a  :=  v ]" := (upd m a v) (at level 12, left associativity) :
  mem_scope.
Add Printing Coercion mem_read.
Notation "m [ a ]" := (mem_read m a) (at level 13, no associativity) :
  mem_scope.
End MemNotations.
