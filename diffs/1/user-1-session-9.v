...
From Tactical Require Import Propositional ExistentialVariants.
...
Inductive pred := mkPred {predApply :> mem -> Prop}.
Implicit Types (a : A) (v : V).
Implicit Types (m : mem) (p : pred).
Definition pimpl p1 p2 := forall m, p1 m -> p2 m.
Infix "===>" := pimpl (at level 60, no associativity).
#[global]Instance pimpl_preorder : (PreOrder pimpl).
firstorder.
Qed.
Definition piff p1 p2 := forall m, p1 m <-> p2 m.
Infix "===" := piff (at level 60, no associativity).
#[global]Instance piff_equivalence : (Equivalence piff).
firstorder.
Qed.
Theorem pimpl_to_piff p1 p2 : p1 ===> p2 -> p2 ===> p1 -> p1 === p2.
Proof.
firstorder.
Qed.
Theorem piff_to_pimpl p1 p2 : p1 === p2 -> p1 ===> p2 /\ p2 ===> p1.
Proof.
firstorder.
Qed.
Theorem pred_apply p m : p m -> predApply (mkPred p) m.
Proof.
(simpl; auto).
Qed.
 ...
 t :=
  autounfold with pred;
   repeat
    ...
    | |- _ <-> _ => split
    | H:_ + _ # _
      |- _ => apply disjoint_sym1 in H; apply union_disjoint_elim in H
    | H:_ # _ + _ |- _ => apply union_disjoint_elim in H
    | _ => progress cbn[predApply]
    | _ => progress propositional
    | _ => progress autorewrite with mem
    | _ => solve [ eauto  10 ]
    end.
Theorem pred_impl_apply p1 p2 m : p1 m -> p1 ===> p2 -> p2 m.
Proof.
firstorder.
Qed.
Theorem pred_iff_apply p1 p2 m : p1 m -> p1 === p2 -> p2 m.
Proof.
firstorder.
Qed.
...
t.
Qed.
Theorem emp_to_lift : forall P : Prop, P -> emp === lift P.
Proof.
t.
Qed.
Definition star p1 p2 : pred :=
  mkPred (fun m => exists m1 m2, p1 m1 /\ p2 m2 /\ m1 # m2 /\ m = m1 + m2).
Infix "*" := star.
...
Theorem star_comm p1 p2 : p1 * p2 === p2 * p1.
Proof.
t.
Qed.
Theorem star_assoc p1 p2 p3 : p1 * (p2 * p3) === p1 * p2 * p3.
Proof.
t.
(repeat
  match goal with
...
Qed.
Theorem star_emp_r p : p * emp === p.
Proof.
t.
(descend; intuition eauto).
t.
Qed.
Theorem star_emp_l p : emp * p === p.
Proof.
(rewrite star_comm).
(apply star_emp_r).
Qed.
Theorem lift_star : forall P1 P2, lift P1 * lift P2 === lift (P1 /\ P2).
Proof.
t.
Qed.
Theorem lift_applied P p m : predApply (lift P * p) m -> P /\ p m.
Proof.
t.
Qed.
Theorem impl_with_lift (P : Prop) p1 p2 :
  (P -> p1 ===> p2) -> lift P * p1 ===> p2.
Proof.
t.
Qed.
Theorem lift_equiv : forall P1 P2, P1 <-> P2 -> lift P1 === lift P2.
Proof.
t.
Qed.
Ltac instance_t := unfold Proper, "==>", Basics.impl, pointwise_relation; t.
#[global]Instance lift_respects_iff : (Proper (iff ==> piff) lift).
Proof.
instance_t.
Qed.
Theorem lift_impl : forall P1 P2 : Prop, P1 -> P2 -> lift P1 ===> lift P2.
Proof.
t.
Qed.
#[global]Instance lift_respects_impl : (Proper (Basics.impl ==> pimpl) lift).
Proof.
...
t.
Qed.
Theorem pred_ex_elim T (p : T -> pred) m : pred_ex p m -> exists x, p x m.
Proof.
exact id.
Qed.
...
Qed.
#[global]
Instance pred_ex_respects_impl  T:
 (Proper (pointwise_relation _ pimpl ==> pimpl) (pred_ex (T:=T))).
Proof.
instance_t.
Qed.
#[global]
Instance pred_ex_respects_iff  T:
...
Qed.
#[global]
Instance pred_forall_respects_impl  T:
 (Proper (pointwise_relation _ pimpl ==> pimpl) (pred_forall (T:=T))).
Proof.
instance_t.
Qed.
#[global]
Instance pred_forall_respects_iff  T:
...
Qed.
Context `{Aeq : EqDec A}.
Definition ptsto a v : pred := mkPred (fun m => m = singleton a v).
Hint Unfold ptsto: pred.
Theorem ptsto_strictly_exact a v m1 m2 :
  ptsto a v m1 -> ptsto a v m2 -> m1 = m2.
Proof.
t.
Qed.
Hint Resolve disjoint_different_singleton: core.
Ltac simpl_union := unfold union; cbn[mem_read]; autorewrite with upd; auto.
Theorem upd_ptsto m a v0 v p :
  (star p (ptsto a v0)) m -> (star p (ptsto a v)) (upd m a v).
Proof.
t.
(exists m1,(singleton a v); intuition eauto).
...
simpl_union.
Qed.
Theorem ptsto_eq m a v p : (star p (ptsto a v)) m -> m a = Some v.
Proof.
t.
...
Qed.
#[global]
Instance star_respects_impl : (Proper (pimpl ==> pimpl ==> pimpl) star).
Proof.
instance_t.
Qed.
#[global]Instance piff_subrelation : (subrelation piff pimpl).
Proof.
firstorder.
Qed.
#[global]Instance star_respects_iff : (Proper (piff ==> piff ==> piff) star).
Proof.
...
Qed.
#[global]
Instance piff_pimpl_partial_apply  p:
 (Proper (piff ==> Basics.impl) (pimpl p)).
Proof.
firstorder.
Qed.
#[global]
Instance piff_pimpl_applied : (Proper (piff ==> piff ==> Basics.impl) pimpl).
Proof.
firstorder.
Qed.
#[global]
Instance pimpl_predApply : (Proper (pimpl ==> eq ==> Basics.impl) predApply).
Proof.
instance_t.
Qed.
#[global]Instance piff_predApply : (Proper (piff ==> eq ==> iff) predApply).
Proof.
...
Notation "a |-> v" := (ptsto a v) (at level 35, no associativity) :
  pred_scope.
Notation "[ P ]" := (lift P) : pred_scope.
Notation "'exists!' x .. y , p" :=
  (pred_ex (fun x => .. (pred_ex (fun y => p)) ..))
  (at level 200, x binder, y binder) : pred_scope.
Notation "'forall!' x .. y , p" :=
  (pred_forall (fun x => .. (pred_forall (fun y => p)) ..))
  (at level 200, x binder, y binder) : pred_scope.
End PredNotations.
