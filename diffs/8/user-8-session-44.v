...
Notation "\226\136\163 x , y , .. , z \226\159\169" := (kron .. (kron \226\136\163 x \226\159\169 \226\136\163 y \226\159\169) .. \226\136\163 z \226\159\169)
  (at level 0).
...
Definition bool_to_matrix (b : bool) : Matrix 2 2 :=
  if b then \226\136\1631\226\159\169\226\159\1681\226\136\163 else \226\136\1630\226\159\169\226\159\1680\226\136\163.
Definition bool_to_matrix' (b : bool) : Matrix 2 2 :=
  fun i j =>
  if negb b && (i =? 0) && (j =? 0)
  then 1
...
Definition \207\131y : Square 2 :=
  fun i j => if i + j =? 1 then (- 1) ^ i * Ci else 0.
Definition \207\131z : Square 2 := fun i j => if i =? j then (- 1) ^ i else 0.
Definition phase_shift (\207\149 : R) : Square 2 :=
  fun i j =>
  if (i =? 0) && (j =? 0)
  then 1
  else if (i =? 1) && (j =? 1) then Cexp \207\149 else 0.
...
Lemma control_compat :
  forall n (A B : Matrix n n), A == B -> control A == control B.
Proof.
...
Qed.
Add Parametric Morphism  n : @control n with signature
 mat_equiv ==> mat_equiv as control_mor.
Proof.
(intros).
...
Hint Unfold qubit0 qubit1 hadamard \207\131x \207\131y \207\131z phase_shift control cnot notc
  swap bra ket: U_db.
...
Csimpl.
(rewrite
  (Csum_eq_bounded _ (fun x => (A x (y - n)%nat) ^* * A x (y - n)%nat))).
++
...
(bdestruct\206\169 (n <=? x)).
(rewrite
  (Csum_eq_bounded _ (fun z => (A z (x - n)%nat) ^* * A z (y - n)%nat))).
++
(unfold control, adjoint, Mmult, I in U).
(rewrite Nat.add_0_r).
(rewrite U by lia).
...
Qed.
Hint Rewrite
 hadamard_adj \207\131x_adj \207\131y_adj \207\131z_adj cnot_adj swap_adj braqubit1_adj
 braqubit0_adj control_adj phase_adj : M_db.
...
Qed.
Lemma pure_psd :
  forall (n : nat) (\207\149 : Vector n), positive_semidefinite (\207\149 \195\151 (\207\149) \226\128\160).
Proof.
(intros).
(unfold positive_semidefinite in *).
(intros z).
...
Lemma mixed_state_cond :
  ...
  0 <= b ->
  a + b = 1 ->
  Mixed_State A -> Mixed_State B -> Mixed_State (a .* A .+ b .* B).
Proof.
...
(eapply (Mix_S p); easy).
Qed.
Lemma pure_state_trace_1 :
  forall {n} (\207\129 : Density n), Pure_State \207\129 -> trace \207\129 = 1.
Proof.
...
(simpl; lca).
Qed.
Lemma mixed_state_trace_1 :
  forall {n} (\207\129 : Density n), Mixed_State \207\129 -> trace \207\129 = 1.
Proof.
(intros n \207\129 M).
(induction M).
-
...
Lemma mixed_state_diag_in01 :
  forall {n} (\207\129 : Density n) i,
  Mixed_State \207\129 -> lt i n -> 0 <= fst (\207\129 i i) <= 1.
Proof.
...
Lemma super_I : forall n \207\129, WF_Matrix \207\129 -> super (I n) \207\129 == \207\129.
Proof.
(intros).
(unfold super).
Msimpl.
...
Lemma compose_super_correct :
  ...
  WF_Superoperator g ->
  WF_Superoperator f -> WF_Superoperator (compose_super g f).
Proof.
...
Lemma sum_super_correct :
  ...
  WF_Superoperator f ->
  WF_Superoperator g -> WF_Superoperator (sum_super f g).
Proof.
...
Definition Splus {m} {n} (S T : Superoperator m n) : 
  Superoperator m n := fun \207\129 => S \207\129 .+ T \207\129.
...
Example move_to_0_test_24 :
  ...
  WF_Matrix q1 ->
  WF_Matrix q2 ->
  WF_Matrix q3 -> move_to_0 4 2 \195\151 (q0 \226\138\151 q1 \226\138\151 q2 \226\138\151 q3) == q2 \226\138\151 q0 \226\138\151 q1 \226\138\151 q3.
Proof.
