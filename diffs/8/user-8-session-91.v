...
Lemma fresh_wtype :
  ...
  add_fresh_state w \206\147 = \206\147 ++ add_fresh_state w [].
Proof.
(intros).
...
replace (add_fresh_state w2 (add_fresh_state w1 [])) with
 add_fresh_state w1 [] ++ add_fresh_state w2 []
 by (rewrite <- IHw2; reflexivity).
(rewrite IHw2).
(rewrite IHw1).
(rewrite app_assoc).
reflexivity.
...
(induction v; auto).
Qed.
Lemma maps_to_repeat :
  forall v n W, v < n -> maps_to v (repeat (Some W) n) = Some v.
Proof.
