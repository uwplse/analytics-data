...
Definition denote_gate' (safe : bool) n {w1} {w2} 
  (g : Gate w1 w2) :
...
Proof.
(intros \207\129 M).
(apply mixed_state_compat with
   (A := \207\129 0%nat 0%nat .* I 1 .+ \207\129 1%nat 1%nat .* I 1); 
  try lma).
...
(apply swap_two_base).
Qed.
Definition pad {m} n (A : Square (2 ^ m)) : Square (2 ^ n) :=
  A \226\138\151 I (2 ^ (n - m)).
Require Import Setoid.
Add Parametric Morphism  m n : @pad m n with signature
 mat_equiv ==> mat_equiv as pad_mor.
Proof.
(intros).
(unfold pad).
(unfold kron).
...
replace (2 * 2 * 2 ^ (n - 2))%nat with (2 ^ 1 * 2 ^ 1 * 2 ^ (n - 2))%nat
 by easy.
...
(apply IHi; lia).
Qed.
Lemma swap_to_0_unitary :
  forall i n, (i < n)%nat -> WF_Unitary (swap_to_0 n i).
Proof.
(intros i n L).
(unfold swap_to_0).
(destruct i).
...
reflexivity.
Qed.
Lemma ctrls_to_list_empty :
  forall W lb u, @ctrls_to_list W lb [] u = (O, [], _X).
Proof.
(destruct u; easy).
Qed.
...
Lemma denote_ctrls_qubit :
  ...
  (k < n)%nat ->
  denote_ctrls n u [k] == I (2 ^ k) \226\138\151 \226\159\166 u \226\159\167 \226\138\151 I (2 ^ (n - k - 1)).
Proof.
(intros n u k L).
(remember Qubit as W).
...
(assert
  (H :
   forall n (U : Square n),
   WF_Unitary U -> WF_Unitary (U \226\138\151 \226\136\1631\226\159\169\226\159\1681\226\136\163 .+ I n \226\138\151 \226\136\1630\226\159\169\226\159\1680\226\136\163))).
(intros n U UU).
...
Lemma ctrl_list_to_unitary_unitary :
  forall l r (u : Square 2),
  WF_Unitary u -> WF_Unitary (ctrl_list_to_unitary l r u).
Proof.
(intros l r u Uu).
(induction l).
-
...
(assert
  (H :
   forall n (U : Square n),
   WF_Unitary U -> WF_Unitary (\226\136\1631\226\159\169\226\159\1681\226\136\163 \226\138\151 U .+ \226\136\1630\226\159\169\226\159\1680\226\136\163 \226\138\151 I n))).
(intros n U UU).
(unfold WF_Unitary in *).
Msimpl.
...
specialize (H k I).
specialize
 (ctrl_list_to_unitary_unitary (firstn k lb) (rev (skipn (S k) lb)) (\226\159\166 u \226\159\167))
 as U.
(assert
  (E : (length (firstn k lb) + length (rev (skipn (S k) lb)) + 1 = n)%nat)).
...
Lemma ctrl_list_to_unitary_transpose :
  forall l r u,
  ctrl_list_to_unitary l r (u) \226\128\160 == (ctrl_list_to_unitary l r u) \226\128\160.
Proof.
(intros l r u).
(induction l).
(simpl).
...
Lemma denote_ctrls_transpose :
  ...
  (forall x, In x li -> x < n)%nat ->
  (length li = \226\159\166 W \226\159\167)%nat ->
  denote_ctrls n (trans u) li == (denote_ctrls n u li) \226\128\160.
Proof.
(intros).
(unfold denote_ctrls).
...
specialize (H _ I).
restore_dims
 repeat rewrite rev_length, skipn_length, firstn_length, L, repeat_length;
  lia.
...
replace (2 ^ n)%nat with (2 ^ n0 * 2 * 2 ^ (n - n0 - 1))%nat
 by unify_pows_two.
(repeat apply kron_unitary; try apply id_unitary;
  try apply unitary_gate_unitary).
...
Definition apply_discard {n} (k : nat) :
  Superoperator (2 ^ n) (2 ^ (n - 1)) :=
...
Definition apply_measQ {n} (k : nat) : Superoperator (2 ^ n) (2 ^ n) :=
  Splus (super (I (2 ^ k) \226\138\151 \226\136\1630\226\159\169\226\159\1680\226\136\163 \226\138\151 I (2 ^ (n - k - 1))))
    (super (I (2 ^ k) \226\138\151 \226\136\1631\226\159\169\226\159\1681\226\136\163 \226\138\151 I (2 ^ (n - k - 1)))).
Definition apply_gate {n} {w1} {w2} (safe : bool) 
  (g : Gate w1 w2) (l : list nat) :
...
(destruct (operator_sum_decomposition [I i \226\138\151 \226\159\1680\226\136\163 \226\138\151 I j; I i \226\138\151 \226\159\1681\226\136\163 \226\138\151 I j])
  as [WFS _]).
...
(intros n i j \207\129 E M).
(destruct
  (operator_sum_decomposition [I i \226\138\151 \226\136\1630\226\159\169\226\159\1680\226\136\163 \226\138\151 I j; I i \226\138\151 \226\136\1631\226\159\169\226\159\1681\226\136\163 \226\138\151 I j])
  as [WFS _]).
(assert
  (OS : outer_sum [I i \226\138\151 \226\136\1630\226\159\169\226\159\1680\226\136\163 \226\138\151 I j; I i \226\138\151 \226\136\1631\226\159\169\226\159\1681\226\136\163 \226\138\151 I j] == I (i * 2 * j))).
{
(unfold outer_sum).
(simpl).
Msimpl.
...
Lemma init0_end_superoperator :
  ...
  (i = 2 ^ n)%nat ->
  Mixed_State \207\129 -> Mixed_State (I i \226\138\151 \226\136\1630\226\159\169 \195\151 \207\129 \195\151 (I i \226\138\151 \226\159\1680\226\136\163)).
Proof.
(intros; subst).
...
Lemma init1_end_superoperator :
  forall (n i : nat) (\207\129 : Square (2 ^ n)),
  (i = 2 ^ n)%nat ->
  Mixed_State \207\129 -> Mixed_State (I i \226\138\151 \226\136\1631\226\159\169 \195\151 \207\129 \195\151 (I i \226\138\151 \226\159\1681\226\136\163)).
Proof.
(intros; subst).
(rewrite <- (kron_1_r \207\129)).
(repeat rewrite kron_mixed_product).
...
Lemma map_same_id :
  forall a l,
  map (fun z : nat * nat => if a =? snd z then (fst z, a) else z)
...
(subst; reflexivity).
Qed.
Lemma swap_list_aux_id :
  forall m n l, swap_list_aux m n (combine l l) == I (2 ^ n).
Proof.
(intros m n l).
...
Definition denote_box (safe : bool) {W1 W2 : WType} 
  (c : Box W1 W2) : Superoperator (2 ^ \226\159\166 W1 \226\159\167) (2 ^ \226\159\166 W2 \226\159\167) :=
...
Definition denote_circuit (safe : bool) {W : WType} 
  (c : Circuit W) (\206\1470 \206\147 : Ctx) :
  Superoperator (2 ^ (\226\159\166 \206\1470 \226\159\167 + \226\159\166 \206\147 \226\159\167)) (2 ^ (\226\159\166 \206\1470 \226\159\167 + \226\159\166 W \226\159\167)) :=
...
Lemma denote_output :
  ...
  \226\159\168 \206\1470 | \206\147 \226\138\169 output p \226\159\169 =
  super (pad (\226\159\166 \206\1470 \226\159\167 + \226\159\166 \206\147 \226\159\167) (denote_pat (subst_pat \206\147 p))).
Proof.
(intros).
(simpl).
(unfold subst_pat).
...
Lemma bounded_pat_le :
  forall W (p : Pat W) n n',
  (n <= n')%nat -> Bounded_Pat n p -> Bounded_Pat n' p.
Proof.
...
Lemma singleton_update :
  forall \206\147 W W' v,
  SingletonCtx v W \206\147 -> SingletonCtx v W' (update_at \206\147 v (Some W')).
Proof.
...
Lemma index_merge_l :
  forall \206\147 \206\1471 \206\1472 n w,
  \206\147 \226\169\181 \206\1471 \226\136\153 \206\1472 -> index \206\1471 n = Some w -> index \206\147 n = Some w.
Proof.
...
Lemma index_merge_r :
  forall \206\147 \206\1471 \206\1472 n w,
  \206\147 \226\169\181 \206\1471 \226\136\153 \206\1472 -> index \206\1472 n = Some w -> index \206\147 n = Some w.
Proof.
(intros \206\147 \206\1471 \206\1472 n w H H0).
...
Lemma remove_at_collision :
  ...
  SingletonCtx n W \206\1471 ->
  \206\147 \226\169\181 \206\1471 \226\136\153 \206\1472 -> size_ctx (remove_at n \206\1472) = size_ctx \206\1472.
Proof.
(intros n).
(induction n).
+
...
Lemma update_none_collision :
  forall n W (\206\147 \206\1471 \206\1472 : Ctx),
  SingletonCtx n W \206\1471 ->
  \206\147 \226\169\181 \206\1471 \226\136\153 \206\1472 -> size_ctx (update_at \206\1472 n None) = size_ctx \206\1472.
Proof.
(intros n).
(induction n).
+
...
Lemma denote_gate_circuit :
  forall {w1} {w2} {w'} (safe : bool) (g : Gate w1 w2) 
    p1 (f : Pat w2 -> Circuit w') (\206\1470 \206\147 \206\1471 \206\1472 : Ctx),
  \206\147 \226\169\181 \206\1471 \226\136\153 \206\1472 ->
  \206\1471 \226\138\162 p1 :Pat ->
...
Qed.
Lemma lookup_maybe_S :
  forall x l, lookup_maybe (S x) (map S l) = lookup_maybe x l.
Proof.
(intros x l).
(induction l; auto).
(simpl).
...
Qed.
Lemma types_pat_no_trail :
  forall \206\147 W (p : Pat W), Valid \206\147 \226\138\162 p :Pat -> trim \206\147 = \206\147.
Proof.
(intros \206\147 W).
gen \206\147.
...
Lemma remove_bit_merge' :
  ...
  \206\147' \226\169\181 singleton v Bit \226\136\153 \206\147 -> remove_pat (bit v) \206\147' = trim \206\147.
Proof.
...
replace (remove_pat (bit (S v)) (Some w :: \206\1470)) with
 Some w :: remove_pat (bit v) \206\1470 by easy.
(simpl).
(erewrite IHmerge_ind; easy).
Qed.
Lemma remove_bit_merge :
...
(destruct v; easy).
Qed.
Lemma subst_pat_trim :
  forall W \206\147 (p : Pat W), subst_pat (trim \206\147) p = subst_pat \206\147 p.
Proof.
(intros W \206\147 p).
(induction p).
-
...
(rewrite H; auto).
Qed.
Add Parametric Relation  W1 W2 : Box W1 W2 @HOAS_Equiv W1 W2 reflexivity
 proved by HOAS_Equiv_refl W1 W2 symmetry proved by 
 HOAS_Equiv_sym W1 W2 transitivity proved by HOAS_Equiv_trans W1 W2 as
 eq_set_rel.
...
Qed.
Lemma inSeq_assoc :
  forall {w1} {w2} {w3} {w4} (c1 : Box w1 w2) (c2 : Box w2 w3)
    (c3 : Box w3 w4), c3 \194\183 c2 \194\183 c1 = (c3 \194\183 c2) \194\183 c1.
Proof.
...
reflexivity.
Qed.
Hint Unfold get_fresh add_fresh_state add_fresh_pat process_gate
  process_gate_state: den_db.
Hint Unfold apply_new0 apply_new1 apply_U apply_unitary denote_ctrls
  apply_meas apply_discard apply_assert0 apply_assert1 compose_super Splus
  swap_list swap_two pad denote_box denote_pat super: den_db.
Hint Unfold get_fresh add_fresh_state add_fresh_pat process_gate
  process_gate_state: vector_den_db.
Hint Unfold apply_new0 apply_new1 apply_U apply_unitary denote_ctrls
  apply_meas apply_discard apply_assert0 apply_assert1 compose_super Splus
  swap_list swap_two pad denote_box denote_pat: vector_den_db.
Ltac
 vector_denote :=
