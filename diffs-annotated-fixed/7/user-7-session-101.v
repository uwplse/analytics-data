...
Proof.
Lemma not_sem_sub__eXrefX_reft : forall (w k : nat) (t : ty), ~ ||-[ w, S k][TExist vX (TRef tX)]<= [TRef t].
Lemma not_sem_sub__eXrefX_reft : forall (w k : nat) (t : ty), ~ ||-[ S w, S k][TExist vX (TRef tX)]<= [TRef t].
Proof.
(induction w).
Abort.
Lemma not_sem_sub__refeXrefX_eYrefrefY : ~ ||- [TRef (TExist vX (TRef tX))]<= [TExist vY (TRef (TRef tY))].
