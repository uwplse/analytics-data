Redirect "/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coqXMDeyv"
...
Definition Specification A T R State :=
  A -> State -> SpecProps T R State.
...
Theorem proc_spec_rx :
  forall `(spec : Specification A T R State) 
...
(match goal with
 | Hexec:exec (rx _) _ _
   |- _ =>
       eapply RExecCrash in Hexec; eauto; eapply H4 in Hexec; eauto
 end).
+
...
(match goal with
 | Hexec:exec (rx _) _ _
   |- _ =>
       apply ExecCrashEnd in Hexec; eapply RExecCrash in Hexec;
        eauto; eapply H4 in Hexec; eauto
 end).
+
(match goal with
  ...
  (forall a state0,
   ...
   proc_spec
     (fun (_ : unit) state =>
      ...
      post := fun r state' => post (spec a state) r state';
      recovered := fun r state' =>
                   recovered (spec a state) r state' |}) p rec abs) ->
  proc_spec spec p rec abs.
Proof.
(unfold proc_spec at 2; intros).
(apply H in H1).
...
Ltac
 spec_case pf :=
  eapply proc_spec_weaken;
   [ solve [ apply pf ] | unfold spec_impl ].
Theorem spec_exec_equiv :
  forall `(spec : Specification A T R State) 
    (p p' : proc T) `(rec : proc R) `(abs : Abstraction State),
  exec_equiv p p' ->
  proc_spec spec p' rec abs -> proc_spec spec p rec abs.
Proof.
...
Theorem ret_spec :
  forall `(abs : Abstraction State) (wipe : State -> State -> Prop)
    `(spec : Specification A T R State) (v : T) 
    (rec : proc R),
...
Redirect
"/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coqCaEI6J"
Print Ltac Signatures.
Timeout 1 Print Grammar tactic.
Add Search Blacklist "Raw" "Proofs".
Set Search Output Name Only.
Redirect
"/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coq5AeGHo"
SearchPattern _.
...
Definition init_abstraction (init : proc InitResult)
  (rec : proc unit) `(abs : Abstraction State)
  (init_sem : State -> Prop) :=
  proc_spec
    (fun (_ : unit) w =>
     {|
     ...
     post := fun r w' =>
             ...
             | Initialized =>
                 exists state,
                   abstraction abs w' state /\ init_sem state
             | InitFailed => True
             end;
     recovered := fun _ w' => True |}) init rec
    (IdAbstraction world).
...
(* Auto-generated comment: At 2019-09-05 17:29:36.410000.*)
