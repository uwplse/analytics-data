...
Definition unifier_of_smi : itree smE void -> itree (testerAppE +' unifyE) unit :=
  ...
  := match (m : itree smE void).(observe) with
     ...
     | VisF (sae|) k =>
         ...
         | ServerApp_Send rx => fun k => r <- embed TesterApp_Recv;; trigger (Unify_Guard rx r);; unifier_of_smi_rec (k tt)
          ...
          loop
            (fun _ =>
             ...
             | Message_Cipher reqCipher =>
                 ...
                 | Some _ => embed Network_Send (Message_Cipher (cipher sk PlainMessage_BadRequest));; ret (inl tt)
                 | None => ret (inl tt)
                 end
             | Message_Plain _ => embed Network_Send (Message_Cipher (cipher sk PlainMessage_BadRequest));; ret (inl tt)
             end) tt;; Tau (network_server (k r) sk)
      | ServerApp_Send r =>
          fun k => embed Network_Send (Message_Cipher (cipher sk (PlainMessage_AppData r)));; Tau (network_server (k tt) sk)
      | ServerApp_Fresh => fun k => n <- trigger AppGen_N;; Tau (network_server (k (n : N)) sk)
      end k
  end.
CoFixpoint network_tester {T} (m : itree taE T) (sk : shared_key) : itree (exceptE error +' nondetE +' appgenE +' networkE) T
:=
  match m.(observe) with
  | RetF r => Ret r
  ...
  | VisF e k =>
      ...
      | (||te) =>
          ...
          | TesterApp_Send =>
              ...
              embed Network_Send (Message_Cipher (cipher sk (PlainMessage_AppData r)));; Tau (network_tester (k r) sk)
          end k
      end
  end.
...
Definition showTE (te : sigT (fun X => tE X * X)%type) : string :=
  ...
  | existT _ _ er =>
      let (e, r) := er in
      match e with
      | (ne|) => match ne in (nondetE X) return (X -> _) with
  ...
  | VisF te tk =>
      ...
      | (ne|) =>
          ...
          | Or =>
              fun tk =>
              b <- trigger Or;;
              embed Trace (Event_Tester (existT _ _ ((Or|), b)));; Tau (zip' (tk (negb b) :: others) (tk b) server)
          end tk
      | (|(Throw e|)) => retry e
      | (||hge|) =>
          ...
          | VisF se sk =>
              ...
              | (|(hge|)) =>
                  ...
                  | HsGen_Key =>
                      fun sk =>
                      k <- trigger HsGen_Key;;
                      embed Trace (Event_Server (existT _ _ ((|(HsGen_Key|)), k)));; Tau (zip' others client (sk k))
                  | HsGen_Random =>
                      fun sk =>
                      r <- trigger HsGen_Random;;
                      embed Trace (Event_Server (existT _ _ ((|(HsGen_Random|)), r)));; Tau (zip' others client (sk r))
                  end sk
              | (||age|) =>
                  match age in (appgenE Y) return ((Y -> _) -> _) with
                  | AppGen_N =>
                  ...
                  | AppGen_Request =>
                      fun sk =>
                      r <- trigger AppGen_Request;;
                      embed Trace (Event_Server (existT _ _ ((||AppGen_Request|), r)));; Tau (zip' others client (sk r))
                  end sk
              | (|||se) =>
                  ...
                  | Network_Recv =>
                      ...
                      | Network_Send msg =>
                          fun tk =>
                          embed Trace (Event_Server (existT _ _ ((|||Network_Recv), msg)));; Tau (zip' others (tk tt) (sk msg))
                      end tk
                  | Network_Send msg =>
                      fun sk =>
                      match te in (networkE Y) return ((Y -> _) -> _) with
...
Unset Silent.
End Test.
Redirect "/tmp/coq16819kMH" Print Ltac Signatures.
Timeout 1 Print Grammar tactic.
