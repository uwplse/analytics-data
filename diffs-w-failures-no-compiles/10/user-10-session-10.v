...
Redirect "/var/folders/lm/cpf87_lx21n9bgnl4kr72rjm0000gn/T/coqfwCzEG"
...
Definition cipher {plain_text} : shared_key -> plain_text -> cipher_text plain_text :=
  pair.
Definition decipher {plain_text} (s : shared_key) (c : cipher_text plain_text) :
  option plain_text := let (k, t) := c in if s =? k then Some t else None.
Definition random := N.
  ...
  rec
    (fun st : kvs_state exp =>
     ...
     | Kvs_GET k =>
         ...
         | None =>
             v <- trigger Eval_Var;; embed App_Send (Kvs_OK v);; call (kvs_put k v st)
         end
...
Definition unifier_of_smi :
  ...
  := match (m : itree smE void).(observe) with
     ...
     | VisF (sae|) k =>
         ...
         | App_Recv =>
             fun k =>
             r <- trigger Random_Request;; embed App_Send r;; unifier_of_smi_rec (k r)
         | App_Send rx =>
             fun k =>
             r <- embed App_Recv;; trigger (Unify_Guard rx r);; unifier_of_smi_rec (k tt)
         end k
     | VisF (|ee) k =>
...
Anomaly ""Assert_failure printing/ppconstr.ml:399:14"."
Please report at http://coq.inria.fr/bugs/.
Definition liftState {s a : Type} {m : Type -> Type} `{Functor m} :
  m a -> stateT s m a := @mkStateT s m a \226\136\152 flip compose (flip pair) \226\136\152 flip fmap.
...
Definition Message_Hello (messageRandom : random) (messagePublic : public_key) :
  message := Message_Plain (PlainMessage_Hello messageRandom messagePublic).
Anomaly ""Assert_failure printing/ppconstr.ml:399:14"."
Please report at http://coq.inria.fr/bugs/.
Anomaly ""Assert_failure printing/ppconstr.ml:399:14"."
Please report at http://coq.inria.fr/bugs/.
...
Definition network_of_app {nE} `{networkE -< nE} `{exceptE error -< nE} 
  ...
  | (ae|) =>
      ...
      | App_Send data =>
          embed Network_Send (Message_Cipher (cipher k (PlainMessage_AppData data)))
      end
...
CoFixpoint match_event {X} (e0 : networkE X) (x0 : X) (t : itree tE unit) : 
itree tE unit :=
...
Redirect "/var/folders/lm/cpf87_lx21n9bgnl4kr72rjm0000gn/T/coqtcCzhj"
Print Ltac Signatures.
Timeout 1 Print Grammar tactic.
Timeout 1 Print LoadPath.
...
Timeout 1 Print Grammar tactic.
Timeout 1 Print LoadPath.
Timeout 1 Check @exec_test.
Timeout 1 Check @exec_test.
Timeout 1 Check @exec_test.
Timeout 1 Check @exec_test.
Timeout 1 Check @exec_test.
Timeout 1 Check @exec_test.
Timeout 1 Check @tester.
