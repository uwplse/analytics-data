...
Unset Silent.
Set Diffs "off".
Timeout 1 Check @N.lcm_least.
Timeout 1 Check @MailServer.Lock.
Timeout 1 Check @MailServer.Lock.
Set Printing Width 104.
Inductive compile_mail_base {gm : GoModel} : forall {T}, proc Mail.Op T \226\134\146 proc GoLayer.Op T \226\134\146 Prop :=
  ...
  | cm_lock : forall uid, compile_mail_base (Call (Mail.Lock uid)) (MailServer.Lock uid)
...
Redirect "/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coqfa7Zoi" Print Ltac Signatures.
Timeout 1 Print Grammar tactic.
Add Search Blacklist "Raw" "Proofs".
Set Search Output Name Only.
Redirect "/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coqD4Atl8" SearchPattern _.
...
Redirect "/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coqUo6Lpu" Print Ltac Signatures.
Timeout 1 Print Grammar tactic.
Add Search Blacklist "Raw" "Proofs".
Set Search Output Name Only.
Redirect "/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coqOtbJIv" SearchPattern _.
Remove Search Blacklist "Raw" "Proofs".
Unset Search Output Name Only.
...
Unset Silent.
Qed.
Redirect "/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coq9hkSnS" Print Ltac Signatures.
Timeout 1 Print Grammar tactic.
Add Search Blacklist "Raw" "Proofs".
Set Search Output Name Only.
Redirect "/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coqpwzoFy" SearchPattern _.
Remove Search Blacklist "Raw" "Proofs".
Unset Search Output Name Only.
Set Silent.
...
iDestruct (@ghost_var_agree (discreteC (gset string)) \206\163 with "Hauth [$]") as % Heq_dom.
...
iDestruct "Htmp" as ( tmps_map' ) "(Hdir&Hauth&Hpaths)".
iDestruct (@ghost_var_agree (discreteC (gset string)) \206\163 with "[$] [$]") as % Heq_dom.
(rewrite <- Heq_dom).
...
-
wp_bind.
(<ssreflect_plugin::ssrtclseq@0> destruct (nth_error ltmps i) as [curr_name| ] eqn:Heq_curr_name ;
 last  first).
{
exfalso.
...
(<ssreflect_plugin::ssrtclseq@0> iApply (wp_sliceRead with "[$]") ; first  eauto).
iIntros "!> Hs".
wp_bind.
iInv "Hinv" as "H".
clear \207\131.
...
(<ssreflect_plugin::ssrtclseq@0>
 assert (tmps \226\136\150 list_to_set (take (i + 1) ltmps) = tmps \226\136\150 (list_to_set (take i ltmps) \226\136\170 {[curr_name]}))
  as -> ; last  by auto).
f_equal.
(eapply nth_error_split in Heq_curr_name as (l1, (l2, (Hsplit, Hlen')))).
...
iAssert ([\226\136\151 map] k\226\134\166_ \226\136\136 \207\1311a.(messages), \226\136\131 \206\1790 : gname, \226\140\156\206\147 !! k = Some \206\1790\226\140\157)%I with "[H\206\147]" as "#H\206\147'".
{
...
(<ssreflect_plugin::ssrtclseq@0> iAssert
 ([\226\136\151 map] uid\226\134\166lm \226\136\136 \207\1311a.(messages), MsgInv \206\147 [] uid (MUnlocked, \226\136\133) false)%I with "[Hdirs Hdirlocks]" as
 "H" ; last  first).
{
iModIntro.
...
iExists _ , _.
iFrame.
(<ssreflect_plugin::ssrtclseq@0> assert (x = (MUnlocked, \226\136\133)) as -> ; last  by auto).
...
iAssert ([\226\136\151 map] k\226\134\166y \226\136\136 base.delete SpoolDir \207\1311c.(fs).(dirents), k \226\134\166 \226\136\133)%I with "[Hdirs]" as "Hdirs".
{
(<ssreflect_plugin::ssrtclseq@0> iApply big_sepM_mono ; last  eauto).
...
move : {}Hdirs_delete {}.
rewrite /base.delete.
(<ssreflect_plugin::ssrtclintros@0> generalize (map_delete SpoolDir \207\1311c.(fs).(dirents)) =>dirs).
...
(<ssreflect_plugin::ssrtclseq@0> iDestruct (big_sepM_delete with "Hdirlocks") as "(Hlock&Hdirlocks)" ;
 first  eauto).
(<ssreflect_plugin::ssrtclseq@0> iDestruct (big_sepM_delete with "Hdirs") as "(Hdir&Hdirs)" ; first 
 eauto).
...
rewrite dom_delete_L.
rewrite Hdom.
(assert (Hnin : UserDir k \226\136\137 (set_map UserDir (dom (gset uint64) m) : gset string))).
{
rewrite elem_of_map.
(intros (k0, (Heq, Hin'))).
...
iDestruct "H" as ( \206\147 H\206\147dom ) "H\206\147".
iAssert ([\226\136\151 map] k\226\134\166_ \226\136\136 \207\131.(messages), \226\136\131 \206\1790 : gname, \226\140\156\206\147 !! k = Some \206\1790\226\140\157)%I with "[H\206\147]" as "#H\206\147'".
{
(<ssreflect_plugin::ssrtclseq@0> iApply big_sepM_mono ; last  eauto).
iIntros ( ? ? ? ) "H".
(iDestruct "H" as ( ? ) "(?&?)"; eauto).
...
(<ssreflect_plugin::ssrtclseq@0> iSplitR "Hfrag_tmp Hspoollock" ; last  first).
{
iExists _.
...
iExists [].
rewrite /HeapInv.
(<ssreflect_plugin::ssrtclseq@0> iSplitR "Htmp Hdir Hauth_tmp Hpaths" ; last  first).
{
(iSplitL ""; auto).
iExists _.
iFrame.
...
iClear "Hheap".
(assert
  (((set_map UserDir (dom (gset uint64) \207\131.(messages)) \226\136\170 {[SpoolDir]}) \226\136\150 {[SpoolDir]} : gset string) =
   set_map UserDir (dom (gset uint64) \207\131.(messages))) as ->).
{
set_solver.
}
...
iExists _ , _.
iFrame.
(<ssreflect_plugin::ssrtclseq@0> iMod (@inv_alloc \206\163 go_invG execN _ _ with "[-]") ; last  eauto).
iNext.
iExists _.
iFrame.
Qed.
...
(<ssreflect_plugin::ssrtclseq@0> iSplitR "Htmp Hdir Hpaths Hspoollock" ; last  first).
{
iSplitL "".
{
by iApply big_sepDM_empty.
    ...
    \226\134\146 compile_rel_proc_seq mail_impl esa esc
      \226\134\146 \194\172 proc_exec_seq Mail.l esa (rec_singleton (Ret ())) (1, \207\1311a) Err
        \226\134\146 \226\136\128 \207\1312c (res : T),
            proc_exec_seq GoLayer.Go.l esc (rec_singleton MailServer.Recover) (1, \207\1311c) (Val \207\1312c res)
            \226\134\146 \226\136\131 \207\1312a, proc_exec_seq Mail.l esa (rec_singleton (Ret tt)) (1, \207\1311a) (Val \207\1312a res).
Proof.
(apply mR.R.crash_refinement_seq).
Qed.
Unset Silent.
Print Assumptions mail_crash_refinement_seq.
Redirect "/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coqiTb0SZ" Print Ltac Signatures.
Timeout 1 Print Grammar tactic.
Add Search Blacklist "Raw" "Proofs".
Set Search Output Name Only.
Redirect "/var/folders/5x/1mdbpbjd7012l971fq0zkj2w0000gn/T/coqU4hyw5" SearchPattern _.
Remove Search Blacklist "Raw" "Proofs".
Unset Search Output Name Only.
Timeout 1 Print LoadPath.
